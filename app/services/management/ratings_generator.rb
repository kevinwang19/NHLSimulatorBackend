    ...
    class RatingsGenerator
        ...
        def calculate_skater_ratings(skater_stats, player_position, team_goals_against_avg, team_shots_against_per_game, team_shutouts_per_game)
            ...
            if skater_stats
                ...
                offensive_rating += (player_position != "D" ? 
                    (skater_stats.avgToi.is_a?(String) && skater_stats.avgToi.match?(/^\d{1,2}:\d{2}$/) ?
                        (time_to_decimal(skater_stats.avgToi) * offensive_weights[:fwdAvgToi]) : 
                        (skater_stats.avgToi * offensive_weights[:fwdAvgToi])) :
                    (skater_stats.avgToi.is_a?(String) && skater_stats.avgToi.match?(/^\d{1,2}:\d{2}$/) ?
                        (time_to_decimal(skater_stats.avgToi) * offensive_weights[:defAvgToi]) : 
                        (skater_stats.avgToi * offensive_weights[:defAvgToi]))
                )
                ...
                defensive_rating += (skater_stats.avgToi.is_a?(String) && skater_stats.avgToi.match?(/^\d{1,2}:\d{2}$/) ?
                    (time_to_decimal(skater_stats.avgToi) * defensive_weights[:avgToi]) : 
                    (skater_stats.avgToi * defensive_weights[:avgToi]))
