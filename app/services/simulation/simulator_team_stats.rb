class SimulatorTeamStats
    ...
    def save_team_stats_win(simulation_id, team_id, goals_scored, goals_allowed, pp_goals_scored, num_pps, pk_goals_allowed, num_pks)
        ...
        division_rank, conference_rank, league_rank, is_wildcard, is_presidents = get_team_standing_stats(team_id)
        ...
        team_stat.update(
            ...
            goalsForPerGame: (team_stat.goalsFor + goals_scored) / (team_stat.gamesPlayed + 1),
            goalsAgainst: team_stat.goalsAgainst + goals_allowed,
            goalsAgainstPerGame: (team_stat.goalsAgainst + goals_allowed) / (team_stat.gamesPlayed + 1),
    ...
    def get_team_special_teams_stats(team_stat, pp_goals_scored, num_pps, pk_goals_allowed, num_pks)
        ...
        new_pp_pctg = new_total_pp_goals / new_total_pps.to_f
        ...
        new_pk_pctg = new_total_pk_successes / new_total_pks.to_f
    ...
    def get_team_standing_stats(team_id)
        ...
        division_team_ids = Team.where(division: division).map(&:teamID)
        conference_team_ids = Team.where(conference: conference).map(&:teamID)
